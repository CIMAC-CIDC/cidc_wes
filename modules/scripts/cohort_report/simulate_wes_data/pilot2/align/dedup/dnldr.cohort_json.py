...
def main():
    ...
    #optparser.add_option("-g", "--ext", help="filename extension to key in on", default=None)
    ...
    for l in f:
        ...
        if subdir in check_list and subdir not in added_list:
        #if subdir in check_list and path not in added_list:
            ...
            #Download bai
            bai_path = "%s.bai" % path
            cmd = "gsutil -m cp %s %s" % (bai_path, options.out_dir)
            cmd = cmd.split(" ")
            output, err = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
