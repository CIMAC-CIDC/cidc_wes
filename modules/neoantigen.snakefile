...
def getVCF_file(wildcards):
    ...
    #Default return
    ret = "analysis/somatic/%s/%s_%s.output.twist.neoantigen.vep.vcf" % (run, run, caller)

    rna = config.get('rna', None)
    
    if rna and rna.get(tumor_sample, None):
        #NOT to be confused with analysis/rna/{run}/{run}_{caller}...same!!
        ret = "analysis/somatic/%s/%s_%s.output.twist.neoantigen.vep.rna.vcf" % (run, run, caller)
...
rule neoantigen_bzipAndtabix: # used in RNA 
    input:
        "analysis/somatic/{run}/{run}_{caller}.output.twist.neoantigen.vep.vcf"
    output:
        gz="analysis/somatic/{run}/{run}_{caller}.output.twist.neoantigen.vep.vcf.gz",
        tbi="analysis/somatic/{run}/{run}_{caller}.output.twist.neoantigen.vep.vcf.gz.tbi",
    group: "neoantigen"
    benchmark:
...
rule neoantigen_expression_annotator:
    ...
    input:
        vcf="analysis/rna/{run}/{run}_{caller}.output.twist.neoantigen.vep.rna.vcf",
        salmon="analysis/neoantigen/{run}/{run}.format.quant.sf",
    output:
        "analysis/somatic/{run}/{run}_{caller}.output.twist.neoantigen.vep.rna.vcf"
    params:
...
rule neoantigen_getNeoantigenList:
    ...
    params:
        include_rna = "-r" if config.get('rna') else ""
    output:
    ...
    group: "neoantigen"
    shell:
        "cidc_wes/modules/scripts/neoantigen_getNeoantgnList.py -f {input} -o {output} {params.include_rna}"
