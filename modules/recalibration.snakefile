...
def recalibration_targets(wildcards):
    ...
    if not config.get('tumor_only'): #Only run when we have normals
        for run in config['runs']:
            ...
            #output file map #2
            ls.append("analysis/corealignments/%s/%s.corealignment.output.yaml" % (run,run))
    return ls

def recalibration_output_files_sample(wildcards):
    """returns a list of filepaths generated by this module to store 
    in the CIDC for a given sample 

    NOTE: re-using this fn for both the runs rule and the sample rule!
    """
    ls = []
    sample = wildcards.sample
    ls.append("analysis/align/%s/%s_recalibrated.bam" % (sample,sample))
    ls.append("analysis/align/%s/%s_recalibrated.bam.bai" % (sample,sample))
    return ls

def recalibration_output_files_run(wildcards):
    """returns a list of filepaths generated by this module to store 
    in the CIDC for a given sample 
...
rule recalibration_make_file_map_sample:
    input:
