#module: Purity PostProcessing styeps by Facets in R
#import os
...
def purityprocessing_targets(wildcards):
   ...
   for run in config['runs']:
      ls.append("analysis/germlineVariants/%s/%spurity_postprocessed_results.txt"" % (run,run))
...
rule purityplots_postprocessing:
  ...
  shell:
    "Rscipt --vanilla cidc_wes/modules/scripts/facets_plot.R {input} {output.output_dir} {params.runname}"
