#Len Taing (TGBTG) 2022- based off of wes_monitor_logger.py
"""only used by /home/taing/utils/wes_automator_tx.monitor.sh, sends the 
update/<instance_name>?transfer_complete=COMPLETE or ERROR
Signal
"""
import requests
import sys
...
def log_handler(msg):
    ...
    #RUNS WHEN JOBS COMPLETE, INCLUDING RUN COMPLETION 
        ...
        if step_count == msg["total"]:
            #print("we did it!")
            try:
                r=requests.put("http://%s/update/%s" % (SERVER_IP_PORT,INSTANCE_NAME), data={'transfer_status':"COMPLETE"})
            except:
                print("WES Monitor server down %s" % SERVER_IP_PORT)
        #DONT' need this
        # else:
        ...
        try:
            r=requests.put("http://%s/update/%s" % (SERVER_IP_PORT,INSTANCE_NAME), data={'transfer_status':"ERROR"})
        except:
            print("WES Monitor server down %s" % SERVER_IP_PORT) 
